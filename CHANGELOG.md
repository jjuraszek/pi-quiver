...
- doc_to_md: build the LibreOffice `-env:UserInstallation` value with
  `pathToFileURL()` instead of `file://` + raw path. A profile dir with a
  space (or a Windows drive path) produced an invalid URI, soffice aborted,
  and every DOCX/PPTX conversion failed (#12).
