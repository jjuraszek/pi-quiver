...
bin/pi-quiver.ts                          # pi-quiver CLI source (fetch subcommand); published as esbuild-built dist/, not committed
skills/, .claude-plugin/                  # Claude Code plugin + fetch skill; invisible to pi (explicit pi.extensions manifest), excluded from the npm tarball
