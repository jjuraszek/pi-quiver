...
provider-stall-watchdog.ts                 # watchdog extension (entry in pi.extensions; OFF by default; human TUI only)
...
- **`provider-stall-watchdog` runtime boundary.** OFF by default; only confirmed human interactive TUI runs can arm it. JSON/RPC/print/subagent runs remain inert by activation, not environment or session lineage. Policy D offers each stall to Pi's retry loop up to `maxStallRetries` times (default = layered `retry.maxRetries`, else 3), verified with Pi 0.80.10; unavailable retry falls back to manual resubmission. See `README.md` for settings and operational behavior.
